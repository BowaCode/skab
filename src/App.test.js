...
const APP_NAMESPACE_ID = "skab-chat-v1.22-groups-membership-fixes"; // Updated version
    ...
    try {
      if (action === 'addFriend') {
        ...
        await setDoc(frRef, {
            ...
            toUid: targetUser.uid, toName: targetUser.displayName || "User", toPhotoURL: targetUser.photoURL || "", toDiscriminator: targetUser.discriminator || "0000",
  ...
  const createGroup = useCallback(async (groupName) => {
    ...
    try {
      const groupRef = await addDoc(collection(db, "groups"), {
        name: groupName.trim(),
        creatorUid: user.uid,
        creatorName: user.displayName || "A Skab User",
        createdAt: serverTimestamp(),
        members: [user.uid], 
        memberInfo: { 
          [user.uid]: {
            displayName: user.displayName || "A Skab User",
...
const ChatWindow = () => { 
  ...
  const handleSendMessage = useCallback(async (e) => {
    ...
    const messageData = { 
        ...
        userDisplayName: user.displayName || user.email.split('@')[0] || "A Skab User", 
    ...
    try {
      ...
      if (currentChat.type === 'dm') {
        ...
        await updateDoc(doc(db, "groups", currentChat.id), {
            ...
            lastMessageSenderName: user.displayName || "A Skab User",
            lastMessageTimestamp: serverTimestamp()
        });
